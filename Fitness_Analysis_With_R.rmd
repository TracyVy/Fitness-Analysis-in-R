...
2. a. There are a few observations with sedentary minutes to be 1440, which means the users were idle for the entire 24 hours of the day. This has a very low possibility of occurrence, unless the user is gravely ill. Keeping the data will yield inaccurate insights. 
...
2. b. With that same judgment, I will also filter out all observations with 0 (zero) TotalActiveMins. Again, taking steps to take care of personal things would trigger the active minutes counter, hence an observation of 0 (zero) active minutes has a very low possibility of occurence.
...
3. a. Create new column "TotalActiveMins" and verify if the sum of "TotalActiveMins" and "SedentaryMins" is 1440. 1440 is the total minutes in a 24 hour period.
...
3. b. After sorting TotalActive_plus_Sedentary, we find that majority of the observations did not include an entire day's worth of activities. Find out how many observations did not include an entire day of activities.
...
4. Filter out observations that has TotalActive_plus_Sedentary < 1200 or 20 hours of logged activities.
...
```
 We are down to 446 observations, with 30 distinct users. Adequate amount of data to use for analysis.

5. Find the average of total sedentary minutes, SedentaryMinutes, and total active minutes, TotalActiveMins, and group by User.
...
6. Find the average total minutes each user spends asleep and idle, in bed but not asleep. 
...
7. a. Create a new data frame that merge the average minutes active, asleep, and idle in bed, join by "Id".
```{r new data frame, final_data}
final_data <- 
pivot_AVG_ActSed_SELECT %>%
inner_join(pivot_AVG_Bed_SELECT)
```
7. b. Include a new column, SedentaryDifference, to express the difference between sedentary minutes and total minutes asleep and idle in bed.
...
9. Then group top third of active users in category "Gold", middle third as "Silver", and least active as "Bronze". Create new column for Category - Gold/Silver/Bronze.
```{r }
...
10. Get the average total sleep, idle minutes in bed, active minutes, and sedentary minutes. Group by Category.
```{r}
